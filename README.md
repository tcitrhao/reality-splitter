...
- 当长文默认配置为智谱 GLM 时，自动调用智谱 Web Search 获取结构化来源；每个智谱配置可以独立选择基础版、高级版、搜狗增强版或夸克增强版，短文模式不会调用搜索。

旧版的短文和长文配置会在首次打开时自动迁移到配置库，已有 API Key 不会丢失。连接测试会真实请求一次模型，可能产生极少量 Token 费用。API Key 仍然只保存在本机 `chrome.storage.local`。

智谱长文搜索使用官方 `POST /api/paas/v4/web_search` 接口。插件会从长文中生成最多 3 条、每条不超过 70 字的查询，并按当前智谱模型配置选择 `search_std`、`search_pro`、`search_pro_sogou` 或 `search_pro_quark`。标题、链接、摘要、媒体和日期会交给长文模型核查；搜索摘要只作为外部证据线索，不会被当作系统指令。正常成功路径每篇长文最多产生 3 次搜索调用；如果搜索链路临时失败并触发一次整体重试，极端情况下最多产生 6 次调用。
