...
智谱长文搜索使用官方 `POST /api/paas/v4/web_search` 接口。插件会从长文中生成最多 3 条、每条不超过 70 字的查询，使用高级版 `search_pro` 搜索，并把标题、链接、摘要、媒体和日期交给长文模型核查。搜索摘要只作为外部证据线索，不会被当作系统指令。按官方当前价格，每篇长文最多产生 3 次高级搜索调用。
