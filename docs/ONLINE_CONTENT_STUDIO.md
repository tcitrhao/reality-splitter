...
- 编辑后可以点击编辑区下方的“保存草稿”，当前条目会在左侧标记为“【草稿】”；草稿只保存在当前浏览器。
- 点击“发布到官网”会更新 `main` 分支中的 `content/website-content.json`。
- GitHub Pages 工作流会自动运行，通常几分钟内更新公网网站；如果 Token 只有读取权限，发布按钮会在提交时明确提示并给出 Token 设置入口。

后台提交时会携带最新读取的文件 SHA。如果内容已被其他方式更新，后台会尝试合并远程变化；如果 Token 只有读取权限，GitHub 会拒绝提交，此时需要重新创建或更新 Fine-grained Token 的 `Contents: Read and write` 权限。
